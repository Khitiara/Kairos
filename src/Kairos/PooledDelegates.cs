...
internal static class PooledDelegates
{
    private static class DefaultDelegatePool<T>
        where T : class, new()
    {
        public static readonly ObjectPool<T>
            Instance = new DefaultObjectPool<T>(new DefaultPooledObjectPolicy<T>(), 20);
    }
    ...
    public static Releaser GetPooledSendOrPostCallback<TArg>(Action<object?, TArg> unboundAction, TArg argument,
        out SendOrPostCallback callback)
        => GetPooledDelegate<SendOrPostCallbackWithBoundArgument<TArg>, TArg, Action<object?, TArg>,
            SendOrPostCallback>(unboundAction, argument, out callback);
    ...
    /// except typed such that it can be used to create a pooled <see cref="ConditionalWeakTable{TKey, TValue}.CreateValueCallback"/>.
    /// </summary>
    public static Releaser GetPooledCreateValueCallback<TKey, TArg, TValue>(
        ...
        where TKey : class
        where TValue : class =>
        GetPooledDelegate<CreateValueCallbackWithBoundArgument<TKey, TArg, TValue>, TArg, Func<TKey, TArg, TValue>,
            ConditionalWeakTable<TKey, TValue>.CreateValueCallback>(unboundFunction, argument, out boundFunction);
    ...
    /// <paramref name="argument"/>.</param>
    /// <returns>A disposable <see cref="Releaser"/> which returns the object to the delegate pool.</returns>
    public static Releaser GetPooledFunction<T1, TArg, TResult>(Func<T1, TArg, TResult> unboundFunction, TArg argument,
    ...
    /// <paramref name="argument"/>.</param>
    /// <returns>A disposable <see cref="Releaser"/> which returns the object to the delegate pool.</returns>
    public static Releaser GetPooledFunction<T1, T2, T3, TArg, TResult>(Func<T1, T2, T3, TArg, TResult> unboundFunction,
        TArg argument, out Func<T1, T2, T3, TResult> boundFunction)
    ...
    private sealed class SendOrPostCallbackWithBoundArgument<TArg>
        : AbstractDelegateWithBoundArgument<SendOrPostCallbackWithBoundArgument<TArg>, TArg, Action<object?, TArg>,
            SendOrPostCallback>
    {
        protected override SendOrPostCallback Bind() => s => UnboundDelegate(s, Argument);
    }
    ...
    private sealed class ActionWithBoundArgument<T1, T2, T3, TArg>
        : AbstractDelegateWithBoundArgument<ActionWithBoundArgument<T1, T2, T3, TArg>, TArg, Action<T1, T2, T3, TArg>,
            Action<T1, T2, T3>>
    {
